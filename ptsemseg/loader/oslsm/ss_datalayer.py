...
class DBInterface():
    ...
    def load_items(self):
        self.db_items = []
        if 'image_sets' in self.params.keys():
...
class PairLoaderProcess(Process):
    def __init__(self, name, queue, db_interface, params):
        ...
        if 'shape_divisible' in params.keys():
        ...
        self.first_label_scale = 1.0 if not ('first_label_scale' in params.keys()) else params['first_label_scale']
        ...
        self.deploy_mode = params['deploy_mode'] if ('deploy_mode' in params.keys()) else False
        self.has_cont = params['has_cont'] if ('has_cont' in params.keys()) else False
        if self.bgr:
    ...
    def load_frame(self, player, first_index, second_index):
        ...
        for ind in first_index:
            ...
            image1, label1 = frame1_dict['image'], frame1_dict['mask']#self.__prepross(frame1_dict, shape1)
        ...
        if second_index is not None:
            ...
            image2, label2 = frame2_dict['image'], frame2_dict['mask']#self.__prepross(frame2_dict, self.second_shape)
