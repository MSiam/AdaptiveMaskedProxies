...
#                    mask[mask == -1] = 0
        ...
        if ('mask' in output.keys()) and output['mask'] is not None:
...
class DBPascalItem(DBImageItem):
    ...
    def __init__(self, name, img_path, mask_path, obj_ids, ids_map = None, fold=0, binary=False):
        ...
        for c in range(21):
            if c not in self.ignore_classes:
                DBPascalItem.map_labels[c] = last_class
                last_class += 1

        if DBPascalItem.global_last_class == 0:
            DBPascalItem.global_last_class = last_class
