...
class RandomizedScale(object):
    def __init__(self, rng):
        self.rng = rng

    def __call__(self, img, mask):
        random_scale = random.uniform(self.rng[0], self.rng[1])
        assert img.size == mask.size
        w, h = img.size
        if random_scale >= 1.0 and random_scale <= 1.2:
            return img, mask
        ow = int(w * random_scale) #self.size
        oh = int(h * random_scale) #int(self.size * h / w)
        return (
