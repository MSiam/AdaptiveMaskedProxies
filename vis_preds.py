...
def create_overlay(img, mask, colors):
    im= Image.fromarray(np.uint8(img[:,:,::-1]))
    im= im.convert('RGBA')
...
for f in sorted(os.listdir(main_dir+'gt/')):
    ...
    for i in range(5):
        ...
        sprt_img = cv2.imread(main_dir+'sprt_images/'+f.split('.')[0]+'_shot%d.png'%i)[:, :, ::-1]
        sprt_gt = cv2.imread(main_dir+'sprt_gt/'+f.split('.')[0]+'_shot%d.png'%i, 0)
        ...
        cv2.imwrite('video/fold3/overlay_qry_'+f, overlay_qry_pred[:,:,::-1])
        cv2.imwrite('video/fold3/overlay_sprt_shot%d_'%i+f, overlay_sprt[:,:,::-1])
